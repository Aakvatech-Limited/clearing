...
function openDocumentAttachmentDialog(frm) {
      ...
      frappe.call({
        ...
        callback: function (r) {
          if (r.message && r.message.length > 0) {
            frappe.throw(
              __(
                "This document type has already been attached to this Shipping Line Clearance."
              )
            );
            return;
          }
          ...
          child.document_name = values.document_type;
          child.document_received = 1;
  ...
  // Set query to filter document types
  d.fields_dict.document_type.get_query = function () {
    return {
      filters: {
        linked_document: "Shipping Line Clearance",
      },
    };
  };
  ...
  if (document_type) {
    frappe.call({
      ...
      error: function (err) {
        ...
        frappe.msgprint(
          __("Failed to retrieve document attributes. Please try again.")
        );
      },
    });
  }
